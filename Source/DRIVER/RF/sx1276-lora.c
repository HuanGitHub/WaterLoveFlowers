...
#define RF_MAX_PACKET_SIZE     200      // 发送数据每包最大长度
/*
===============================================================================
...
void SX1276_LoRa_SetDataRate( INT8U rate )
{
	...
	SX1276_LoRa_SetBW( LoRa_DataRate_Table[rate][0] );
	SX1276_LoRa_SetSpreadingFactor( LoRa_DataRate_Table[rate][1] );
	SX1276_LoRa_SetErrorCoding( LoRa_DataRate_Table[rate][2] );
}
/*
===============================================================================
...
*/
    SX1276_WriteReg( REG_LR_DIOMAPPING2, 0xC0);
//    SX1276_SetSwitchTx(  );
    SX1276_LoRa_SetMode( LORA_MODE_TX );

//    while( ! SX1276_READ_DIO0( ) ); //TxDone
    while( !( SX1276_ReadReg( 0x12 ) & ( 1<<3 ) ) ) ;
...
void SX1276_LoRa_StartRx( void )
{
    ...
    SX1276_WriteReg( REG_LR_DIOMAPPING2, 0x40);
...
//    SX1276_WriteReg( REG_LR_FIFOADDRPTR, SX1276_ReadReg( REG_LR_FIFORXBASEADDR ) );
	for( i=0; i<size; i++ )
	{
		*buffer ++ = SX1276_ReadFifo( );
	}
