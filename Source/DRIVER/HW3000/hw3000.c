...
void hw3000_init(hw3000_mode_t hw3000_mode)
{
   ...
   // hw3000_write_reg(0x02, 0xc006);
    hw3000_write_reg(0x14, 0x7e7e);//set SFD First word
    hw3000_write_reg(0x15, 0x7e7e);//set SFD Second word
	//hw3000_write_reg(PKTCFG1,0x01FF); //允许同步字错误个数为0
...
void hw3000_frame_set(hw3000_mode_t hw3000_mode)
{
    ...
    { //direct fifo mode		
		...
		hw3000_write_reg(PKTCTRL, 0x4000);
		hw3000_write_reg(PKTCFG0, 0x8010); //SFD 4bytes, preamble 8 bytes
		hw3000_write_reg(PREACFG, 0x0206);
		hw3000_write_reg(FIFOSTA, 0x0000); //first 2 bytes is length
...
int8_t hw3000_frame_tx(hw3000_mode_t hw3000_mode, hw3000_data_t *txbuf)
{
	...
	if (_hw3000_state != TX)
    {
        ...
        //while(GPIO_ReadInputPin(HW3000_IRQ_PORT,HW3000_IRQ_IO)==SET);
		_hw3000_irq_request = 0;
		_hw3000_state = IDLE;
...
int8_t hw3000_rx_data(hw3000_mode_t hw3000_mode, hw3000_data_t *rxbuf)
{
	...
	if (_hw3000_state == RX) 
    {
        ...
        {
			hw3000_read_fifo(FIFODATA,rxbuf->data, 1);
            rxbuf->len = rxbuf->data[0]-1;
            hw3000_read_fifo(FIFODATA, &rxbuf->data[1], rxbuf->len);
		} 
...
void hw3000_deep_sleep_down(void)
{
	...
	hw3000_write_reg(TRCTRL, 0x0000);
...
void hw3000_deep_sleep_up(void)
{
    ...
    while(!(hw3000_read_reg(INTFLAG) & 0x4000)); //wait for chip ready
    hw3000_freq_set(_hw3000_mode); //from deep sleep/sleep need reconfigure this reg
}
...
uint16_t hw3000_read_reg(uint8_t addr)
{
 ...
 for(i=0;i<16;i++)
 {
  ...
  GPIO_WriteHigh(HW3000_CLK_PORT,HW3000_CLK_IO);
  GPIO_WriteLow(HW3000_CLK_PORT,HW3000_CLK_IO);
  if(GPIO_ReadInputPin(HW3000_SDO_PORT,HW3000_SDO_IO)==SET)
  {
   temp|=0x0001;
  }
  else
